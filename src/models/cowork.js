...
const getCoworkThatContainsNameAsync = (name) => {
  const query = `
    MATCH
      (c:Cowork)-[:IS_FROM]->(ct:City),
      (c:Cowork)-[:IS_FROM]->(co:Country)
...
export async function getCoworkThatContainsName(name) {
  try {
    const coworkByName = await getCoworkThatContainsNameAsync(name);
    return coworkByName;
  } catch (err) {
    console.error(err);
    return err;
